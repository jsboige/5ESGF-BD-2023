...
namespace mysolver
{
    ...
    public class SudokuPermutationsChromosome : SudokuChromosomeBase, ISudokuChromosome
    {
        ...
        public SudokuPermutationsChromosome(GridSudoku targetGridSudoku) : this(targetGridSudoku, 9) { }
        ...
        public SudokuPermutationsChromosome(GridSudoku targetGridSudoku, int length) : this(targetGridSudoku, null, length) { }
        ...
        public SudokuPermutationsChromosome(GridSudoku targetGridSudoku, Dictionary<(int row, int column), List<int>> extendedMask) : this(targetGridSudoku, extendedMask, 9) { }

        /// <param name="targetGridSudoku">the target sudoku to solve</param>
        /// <param name="extendedMask">The cell domains after initial constraint propagation</param>
        /// <param name="length">The number of genes for the sudoku chromosome</param>
        public SudokuPermutationsChromosome(GridSudoku targetGridSudoku, Dictionary<(int row, int column), List<int>> extendedMask, int length) : base(targetGridSudoku, extendedMask, length) { }
        ...
        {
            var toReturn = new SudokuPermutationsChromosome(TargetGridSudoku, ExtendedMask, Length);
            return toReturn;
        }
        ...
        public override IList<GridSudoku> GetSudokus()
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                for (int j = 0; j < 9; j++)
                {
                    sudoku.Cellules[i][j] = listInt[index];
                    index++;
                }
            }
        ...
        public IList<IList<IList<int>>> GetRowsPermutations()
        {
            ...
            if (!_rowsPermutations.TryGetValue(TargetGridSudoku, out var toReturn))
            {
                ...
                {
                    if (!_rowsPermutations.TryGetValue(TargetGridSudoku, out toReturn))
                    {
                        toReturn = GetRowsPermutationsUncached();
                        _rowsPermutations[TargetGridSudoku] = toReturn;
                    }
                }
            }
            return toReturn;
        ...
        /// </summary>
        private static readonly IDictionary<GridSudoku, IList<IList<IList<int>>>> _rowsPermutations = new Dictionary<GridSudoku, IList<IList<IList<int>>>>();
